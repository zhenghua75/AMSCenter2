...
namespace BusiComm
{
	...
	public class EmpBusi
	{
		...
		public bool IsEmpSchAllDayWork(string strShcID,string strCardID,string strClass)
		{
			int recount=Ea.IsEmpSchAllDayWork(strShcID,strCardID,strClass);
			if(recount<1)
			{
				return false;
			}
		...
		public bool IsEmpSchDupClass(string strShcID,string strCardID,string strClass)
		{
			int recount=Ea.IsEmpSchDupClass(strShcID,strCardID,strClass);
			if(recount<1)
			{
				return false;
			}
		...
		public DataTable GetSignSumQuery(Hashtable htpara)
		{
			...
			if(dtEmpList!=null&&dtEmpList.Rows.Count>0)
			{
				...
				for(int i=0;i<dtEmpList.Rows.Count;i++)
				{
					if(strsno=="")
					{
						strEmpList=dtEmpList.Rows[i]["vcEmpName"].ToString()+"("+dtEmpList.Rows[i]["eCount"].ToString()+") ";
				...
				if(dtSchLog!=null&&dtSchLog.Rows.Count>0)
				{
					...
					for(int j=0;j<dtSchLog.Rows.Count;j++)
					{
							...
							if(htCardSignAll!=null)
							{
								...
								if(alCardSignLog!=null)
								{
									...
									foreach(CMSMStruct.SignAtomStruct sastmp in alCardSignLog)
									{
										...
										{
											case "1":
												#region 确认签到有效记录，打卡时间在上班时间前后2.5小时内为有效，上班时间前2.5小时内为不迟到，过了上班时间半小时内为迟到，半小时后为旷工
												if((sastmp.dtSignDate.CompareTo(dtCheckIn.AddHours(-2.5)))>=0&&(sastmp.dtSignDate.CompareTo(dtCheckIn.AddHours(2.5)))<=0)
												{
													if(sls.strSignIn=="")
													{
														sls.strSignIn=sastmp.dtSignDate.ToShortDateString()+" "+sastmp.dtSignDate.ToLongTimeString();
														if((sastmp.dtSignDate.CompareTo(dtCheckIn.AddHours(0.5)))>0)
														{
													...
													else
													{
														...
														if((timeold.CompareTo(sastmp.dtSignDate))>0)
														{
															sls.strSignIn=sastmp.dtSignDate.ToShortDateString()+" "+sastmp.dtSignDate.ToLongTimeString();
															if((sastmp.dtSignDate.CompareTo(dtCheckIn.AddHours(0.5)))>0)
															{
												...
												break;
											case "2":
												#region 确认签退有效记录，打卡时间在下班时间前后2.5小时内为有效，下班时间前为早退，后为不早退
												if((sastmp.dtSignDate.CompareTo(dtCheckOut.AddHours(-2.5)))>=0&&(sastmp.dtSignDate.CompareTo(dtCheckOut.AddHours(2.5)))<=0)
												{
													if(sls.strSignOut=="")
													{
						...
						else
						{
							if(sls.strClass=="休息")
							{
								sls.strSignState="0000000010";
								sls.strSignResult="休息";
							}
							else if(sls.strSignIn==""&&sls.strSignOut=="")
							{
