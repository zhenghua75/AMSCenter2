...
namespace AMSApp.StockControl.Services
{
    ...
    /// Inventory 的摘要说明
    /// </summary>
    public class tbInventory : IHttpHandler, IRequiresSessionState
    {
        ...
        private string GetInvCode(HttpContext context)
        {
            ...
            string end = "";
            if(invCCodes.Length>1)
                end = invCCodes[1];
            using (AMSCM.Models.AMSCM amscm = new AMSCM.Models.AMSCM())
            {
                var inv = amscm.tbInventory.Where(w => w.cnvcInvCCode==invCCode).OrderByDescending(o => o.cnvcInvCode).FirstOrDefault();
                if (inv == null)
                {
                    return begin;
                }
        ...
        private IQueryable<AMSCM.Models.tbInventory> QueryCondition(HttpContext context, AMSCM.Models.AMSCM amscm)
        {
            ...
            var q = from p in amscm.tbInventory select p;
        ...
        private DataTable List2DataTable(HttpContext context, List<AMSCM.Models.tbInventory> lInventory)
        {
            DataTable dt = lInventory.ToDataTable<AMSCM.Models.tbInventory>();
