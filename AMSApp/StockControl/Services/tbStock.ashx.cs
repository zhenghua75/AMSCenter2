...
namespace AMSApp.StockControl.Services
{
    ...
    /// 库存主表
    /// </summary>
    public class tbStock : IHttpHandler, IRequiresSessionState
    {
        ...
        private IQueryable<tbStockMainAndDetail> QueryCondition(HttpContext context, AMSCM.Models.AMSCM amscm)
        {
            //主表
            ...
            var q = from main in amscm.tbStockMain
                    ...
                    orderby  detail.cnnDetailId,main.cnnMainId
        ...
        private string GettbStock(HttpContext context)
        {
            ...
            using (AMSCM.Models.AMSCM amscm = new AMSCM.Models.AMSCM())
            {
                ...
                totalcount = tcount.ToString();
                IQueryable<tbStockMainAndDetail> iq = q.Skip(skitCount)
                    .Take(rows);
                //Exception ex = new Exception(iq.ToString());
                //ExceptionPolicy.HandleException(ex, ServiceHelper.ExceptionPolicy);
                ltbStock = iq.ToList();
                //totalcount = q.Select(s=>s.cnnDetailId).Count().ToString();
                
                //UseApproximationForPerformancesSake
            }
            DataTable dt = List2DataTable(context, ltbStock);
            return ServiceHelper.DataTableToEasyUIDataGridJson(dt, totalcount);

        }
        private bool JudgeIsBalance(AMSCM.Models.AMSCM amscm,int cnnYear, int cnnMonth)
        {
            AMSCM.Models.tbMonthlyBalance tbMonthlyBalance = amscm.tbMonthlyBalance.Find(cnnYear, cnnMonth);
            ...
            try
            {
                using (AMSCM.Models.AMSCM amscm = new AMSCM.Models.AMSCM())
                {
                    using (TransactionScope transaction = new TransactionScope())
                    {
                        ...
                        if (JudgeIsBalance(amscm, cndBusinessDate.Year, cndBusinessDate.Month))
                        {
                        ...
                        //tbStockMain.cndModifyDate = tbStockMain.cndCreateDate;
                        //tbStockMain.cnvcModifier = tbStockMain.cnvcCreaterId;
                        //tbStockMain.cnvcModifierName = tbStockMain.cnvcCreaterName;
                        ...
                        {
                            int icount = (from d in amscm.tbStockMain where d.cndBusinessDate == tbStockMain.cndBusinessDate && d.cnvcDeptId == tbStockMain.cnvcDeptId && d.cnnOperType == (int)StockType.Sell select d).Count();
                            if (icount > 0)
                            {
                        ...
                        if (tbStockMain.cnnOperType == (int)StockType.Sell)
                        {
                            string strret = Sell(amscm, tbStockMain, tbStockMain.cndBusinessDate.ToString("yyyy-MM-dd"), tbStockMain.cnvcDeptId);
                        ...
                        else if (context.Request["AddDetail"] != null)
                        {
                            ...
                            if (bAddDetail)
                            {
                                ...
                                for (int i = 0; i < detailCount; i++)
                                {
                                    ...
                                    AMSCM.Models.tbComputationUnit unitCode = amscm.tbComputationUnit.Find(tbStockDetail.cnvcComUnitCode);
                                    AMSCM.Models.tbComputationUnit mainUnitCode = amscm.tbComputationUnit.Where(w => w.cnvcGroupCode == unitCode.cnvcGroupCode && w.cnbMainUnit).FirstOrDefault();
                        ...
                        {
                            Complete(amscm, tbStockMain, ltbStockDetail);
                        }
                        if (tbStockMain.cnnOperType == (int)StockType.Transfer && ltbStockDetail.Count > 0)
                        {
                            ...
                            Transfer(amscm, tbStockMain, ltbStockDetail, incnvcWhCode, incnvcDeptId);
                        }
                        amscm.SaveChanges();
        ...
        private string Sell(AMSCM.Models.AMSCM amscm,AMSCM.Models.tbStockMain tbStockMain,string strdate,string strdeptid)
        {
            try
            {
 ...
 where CONVERT(varchar(10),dtConsDate,121)='"
                ...
                if (dt.Rows.Count > 0)
                {
                    foreach (DataRow dr in dt.Rows)
                    {
                        ...
                        ServiceHelper.AddStockDetal(amscm, tbStockMain, strcnvcInvCode, strcnvcComUnitCode, dcnnQuantity, strcnvcMainComUnitCode, dcnnMainQuantity, dcnnPrice, dcnnAmount);
                    }
                }
            }
        ...
        private void Complete(AMSCM.Models.AMSCM amscm, AMSCM.Models.tbStockMain tbStockMain, List<AMSCM.Models.tbStockDetail> ltbStockDetail)
        {
            ...
            List<AMSCM.Models.tbBillOfMaterials> lComponentInv = new List<AMSCM.Models.tbBillOfMaterials>();
            //List<AMSCM.Models.tbBillOfMaterials> lComponentInvSemi = new List<AMSCM.Models.tbBillOfMaterials>();
            List<AMSCM.Models.tbBillOfMaterials> lSemiProduct = new List<AMSCM.Models.tbBillOfMaterials>();
            List<AMSCM.Models.tbBillOfMaterials> lSemi = new List<AMSCM.Models.tbBillOfMaterials>();
            foreach (AMSCM.Models.tbStockDetail sd in ltbStockDetail)
            {
                if (!string.IsNullOrEmpty(sd.cnvcInvCode))
                {
                    ...
                    List<AMSCM.Models.tbBillOfMaterials> lComponentInv1 = ServiceHelper.ProcBOM(lbom, sd.cnvcInvCode);
                    foreach (AMSCM.Models.tbBillOfMaterials bom in lComponentInv1)
                    {
                    ...
                    List<AMSCM.Models.tbBillOfMaterials> lSemiProduct1 = ServiceHelper.ProcBOM2(lbom, sd.cnvcInvCode);
                    foreach (AMSCM.Models.tbBillOfMaterials bom in lSemiProduct1)
                    {
                        bom.cnnBaseQtyN = bom.cnnBaseQtyN * sd.cnnMainQuantity / q1;
                    }
            ...
            if (lSemiProduct.Count > 0)
            {
                ...
                var q = (from main in amscm.tbStockMain
                        ...
                        join d1 in amscm.tbInventory on detail.cnvcInvCode equals d1.cnvcInvCode into dd1
                        from inventory in dd1.DefaultIfEmpty()

                        join d2 in amscm.tbComputationUnit on inventory.cnvcProduceUnitCode equals d2.cnvcComunitCode into dd2
                        from uom in dd2.DefaultIfEmpty()

                        join d3 in amscm.tbProductClass on inventory.cnvcInvCCode equals d3.cnvcProductClassCode into dd3
                ...
                lSemi = (from d in lcis
                         ...
                         select new AMSCM.Models.tbBillOfMaterials() { cnvcComponentInvCode = d.cnvcInvCode, cnnBaseQtyN = d.cnnQuantity > d1.cnnQuantity ? d.cnnQuantity : d1.cnnQuantity }
                        ).ToList();
                foreach (var q3 in lSemi)
                {
                    if (!string.IsNullOrEmpty(q3.cnvcComponentInvCode))
                    {
                        ...
                        List<AMSCM.Models.tbBillOfMaterials> lComponentInv1 = ServiceHelper.ProcBOM(lbom, q3.cnvcComponentInvCode);
                        foreach (AMSCM.Models.tbBillOfMaterials bom in lComponentInv1)
                        {
            ...
            if (lComponentInv.Count > 0)
            {
                ...
                AMSCM.Models.tbStockMain tbStockMain2 = new AMSCM.Models.tbStockMain();
                ServiceHelper.SetEntity<AMSCM.Models.tbStockMain, AMSCM.Models.tbStockMain>(tbStockMain, tbStockMain2);
                tbStockMain2.cnnOperType = (int)StockType.Material;
                tbStockMain2.cnnSource = tbStockMain.cnnMainId;
                amscm.tbStockMain.Add(tbStockMain2);
                amscm.SaveChanges();

                AMSCM.Models.tbStockMainLog tbStockMainLog2 = new AMSCM.Models.tbStockMainLog();
                ServiceHelper.SetEntity<AMSCM.Models.tbStockMain, AMSCM.Models.tbStockMainLog>(tbStockMain2, tbStockMainLog2);
                amscm.tbStockMainLog.Add(tbStockMainLog2);
                foreach (var q1 in q)
                {
                    ServiceHelper.AddStockDetal(amscm, tbStockMain2, q1.cnvcInvCode, q1.cnvcSTComUnitCode, -q1.cnnQuantity, q1.cnvcComUnitCode, -q1.cnnMainQuantity, 0, 0);
                }
                
            }
        }
        private void Transfer(AMSCM.Models.AMSCM amscm, AMSCM.Models.tbStockMain tbStockMainOut, List<AMSCM.Models.tbStockDetail> ltbStockDetailOut,string incnvcWhCode,string incnvcDeptId)
        {
            AMSCM.Models.tbStockMain tbStockMain = new AMSCM.Models.tbStockMain();
            ServiceHelper.SetEntity<AMSCM.Models.tbStockMain, AMSCM.Models.tbStockMain>(tbStockMainOut, tbStockMain);
            ...
            foreach (AMSCM.Models.tbStockDetail tbStockDetailOut in ltbStockDetailOut)
            {
                AMSCM.Models.tbStockDetail tbStockDetail = new AMSCM.Models.tbStockDetail();
                ServiceHelper.SetEntity<AMSCM.Models.tbStockDetail, AMSCM.Models.tbStockDetail>(tbStockDetailOut, tbStockDetail);
                tbStockDetail.cnnMainId = tbStockMain.cnnMainId;
                tbStockDetail.cnnQuantity = -tbStockDetail.cnnQuantity;
                tbStockDetail.cnnAmount = -tbStockDetail.cnnAmount;
                amscm.tbStockDetail.Add(tbStockDetail);
            ...
            try
            {
                using (AMSCM.Models.AMSCM amscm = new AMSCM.Models.AMSCM())
                {
                    using (TransactionScope transaction = new TransactionScope())
                    {
                        ...
                        AMSCM.Models.tbStockMain tbStockMain = amscm.tbStockMain.Find(tbStockDetail.cnnMainId);
                        ...
                        if (JudgeIsBalance(amscm, tbStockMain.cndBusinessDate.Year, tbStockMain.cndBusinessDate.Month))
                        {
                            return ServiceHelper.JsonSerializer<JEasyUIResult>(new JEasyUIResult(false, "已月结不能操作！"));
                        }
        ...
        private string updatetbStockMain(HttpContext context)
        {
            try
            {
                using (AMSCM.Models.AMSCM amscm = new AMSCM.Models.AMSCM())
                {
                    AMSCM.Models.tbStockMain tbStockMain = amscm.tbStockMain.Find(Convert.ToInt64(context.Request.Form["cnnMainId"]));
        ...
        private string checktbStockMain(HttpContext context)
        {
            try
            {
                using (AMSCM.Models.AMSCM amscm = new AMSCM.Models.AMSCM())
                {
                    ...
                    AMSCM.Models.tbStockMain tbStockMain2 = amscm.tbStockMain.Where(w => w.cnnSource == tbStockMain.cnnMainId).FirstOrDefault();
        ...
        private string updatetbStockDetail(HttpContext context)
        {
            try
            {
                using (AMSCM.Models.AMSCM amscm = new AMSCM.Models.AMSCM())
                {
                    ...
                    AMSCM.Models.tbStockDetail tbStockDetail = amscm.tbStockDetail.Find(lcnnDetailId);
                    AMSCM.Models.tbStockMain tbStockMain = amscm.tbStockMain.Find(tbStockDetail.cnnMainId);
                    if (JudgeIsBalance(amscm, tbStockMain.cndBusinessDate.Year, tbStockMain.cndBusinessDate.Month))
                    {
                        return ServiceHelper.JsonSerializer<JEasyUIResult>(new JEasyUIResult(false, "已月结不能操作！"));
        ...
        private string removetbStockMain(HttpContext context)
        {
            try
            {
                ...
                using (AMSCM.Models.AMSCM amscm = new AMSCM.Models.AMSCM())
                {
                    AMSCM.Models.tbStockMain tbStockMain = amscm.tbStockMain.Find(lcnnMainId);
                    if (tbStockMain.cnnStatus == (int)StockStatus.Check)
                    {
                    ...
                    List<AMSCM.Models.tbStockDetail> ltbStockDetail = amscm.tbStockDetail.Where(w => w.cnnMainId == lcnnMainId).ToList();
                    foreach (AMSCM.Models.tbStockDetail tbStockDetail in ltbStockDetail)
                    {
        ...
        private string removetbStockDetail(HttpContext context)
        {
            try
            {
                using (AMSCM.Models.AMSCM amscm = new AMSCM.Models.AMSCM())
                {
                    AMSCM.Models.tbStockDetail tbStockDetail = amscm.tbStockDetail.Find(Convert.ToInt64(context.Request.Form["cnnDetailId"]));
                    AMSCM.Models.tbStockMain tbStockMain = amscm.tbStockMain.Find(tbStockDetail.cnnMainId);
                    if (tbStockMain.cnnStatus == (int)StockStatus.Check)
                    {
                        return ServiceHelper.JsonSerializer<JEasyUIResult>(new JEasyUIResult(false, "弃审后才可以删除"));
