$(function () {
    ...
    $('#search-cnvcWhCode').combogrid({
        ...
        { field: 'cnbInvalidComments', title: '是否失效', width: 100, formatter: formatCheckBox }
    ]]
    });
    ...
    $('#cost-dlg').dialog({
        ...
        closed: true,
        shadow: false,
        buttons: '#cost-dlg-buttons'
    });
    $('#cost-cnnYear').combobox(
    {
        url: 'Services/tbMonthlyBalance.ashx?Method=cnnYear',
        valueField: 'id',
        textField: 'text',
...
function StockReportExportToExcel() {
    $('#export-excel').form('submit', {
        ...
        onSubmit: function (param) {
            param.cnvcWhCode = $("#search-cnvcWhCode").combogrid('getValue');
            param.cnvcDeptId = $("#search-cnvcDeptId").combogrid('getValue');
            param.cnvcInvCode = $("#search-cnvcInvCode").combogrid('getValue');
            param.cnnYear = $("#search-cnnYear").combobox("getValue");
            param.cnnMonth = $("#search-cnnMonth").combobox("getValue");
...
function costStockReport() {
    $('#cost-dlg').dialog('open').dialog('setTitle', '计算成本');
    moveDialog('cost-dlg');
}
function StockReportCost() {
    $('#cost-form').form('submit', {
        url: 'Services/StockReport.ashx?Method=compute',
        onSubmit: function () {
            return $(this).form('validate');
        },
        success: function (result) {
