...
$(function () {
    ...
    $('#cnvcComUnitCode').combogrid({
        ...
        columns: [[
                ...
                { field: 'cnbMainUnitComments', title: '是否主计量单位', width: 100 ,formatter:formatCheckBox},
    ...
    $('#search-cnvcComUnitCode').combogrid({
        ...
        columns: [[
                    ...
                    { field: 'cnbMainUnit', title: '是否主计量单位', width: 100 ,formatter:formatCheckBox},
                    { field: 'cniChangRate', title: '换算率', width: 100 }
                ]]
    });
...
function edittbStockMain() {
    ...
    if (row) {
        ...
        if (stockType == "7") {
            ...
            $('#divInWh').show();
            $('#divInDept').show();
            $('#incnvcWhCode').combogrid('setValue', row.cnvcWhCode);
            $('#incnvcWhCode').combogrid('setText', row.cnvcWhCodeComments);
            $('#incnvcDeptId').combogrid('setValue', row.cnvcDeptId);
            $('#incnvcDeptId').combogrid('setText', row.cnvcDeptIdComments);
        }
...
function balance() {
  ...
  msg:'正在月结，请等待'
        ...
        $.ajax({
            url: 'Services/tbMonthlyBalance.ashx',
            type: 'POST',
            data: { 'Method': 'balance', 'cnnYear': row.cnnYear, 'cnnMonth': row.cnnMonth },
            datatype: 'json',
            timeout: 600000,
            success: function (result) {
                //alert(result);
