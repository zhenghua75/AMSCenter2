...
//						SqlHelper.ExecuteNonQuery(con,tran,CommandType.Text,sql4);
		...
		public DataTable GetDailySaleChart(string strMonth)
		{
			DataTable dtout=new DataTable();
			try
			{
				string sql="select vcDeptID,convert(char(8),dtConsDate,112) as SaleDay,round(sum(nFee)/10000,2) as SaleFee from tbConsItemOther where convert(char(6),dtConsDate,112)='"+strMonth+"' group by vcDeptID,convert(char(8),dtConsDate,112) order by vcDeptID,convert(char(8),dtConsDate,112)";
				dtout=SqlHelper.ExecuteDataTable(con,CommandType.Text,sql);
			}
			catch (Exception e) 
			{
