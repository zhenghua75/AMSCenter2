...
namespace DataAccess
{
	...
	public class EmpAcc
	{
		...
		public int ModEmployee(CMSMStruct.EmployeeStruct esold,CMSMStruct.EmployeeStruct esnew)
		{
			...
			if(esold.strInDate!=esnew.strInDate)
			{
				if(strCondition!="")
				{
					strCondition+=",dtInDate='"+esnew.strInDate+"'";
				}
				else
				{
					strCondition+=" dtInDate='"+esnew.strInDate+"'";
		...
		public DataTable GetAllEmpName(string strDeptID,string strSchID)
		{
			...
			try
			{
				string sql1="SELECT vcEmpName FROM tbEmployee where vcFlag='0' and vcDeptID='"+strDeptID+"' and vcCardID not in(select vcCardID from tbEmpSchLog where vcSchID='"+strSchID+"' and vcClass in('È«ÈÕ°à','ÐÝÏ¢'))";
				dtemp=SqlHelper.ExecuteDataTable(con,CommandType.Text,sql1);
			}
			catch (Exception e) 
			{
		...
		public int IsEmpSchAllDayWork(string strSchID,string strCardID,string strClass)
		{
			int strreturn=0;
			try
			{
				...
				if(strClass=="È«ÈÕ°à")
				{
					sql1="select count(*) from tbEmpSchLog where vcSchID='"+strSchID+"' and vcCardID='"+strCardID+"' and vcClass<>'È«ÈÕ°à'";
				}
				else
				{
					if(strClass=="ÐÝÏ¢")
						sql1="select count(*) from tbEmpSchLog where vcSchID='"+strSchID+"' and vcCardID='"+strCardID+"' and vcClass<>'ÐÝÏ¢'";
					else
						sql1="select count(*) from tbEmpSchLog where vcSchID='"+strSchID+"' and vcCardID='"+strCardID+"' and vcClass in('È«ÈÕ°à','ÐÝÏ¢')";
				}
		...
		public int IsEmpSchDupClass(string strSchID,string strCardID,string strClass)
		{
			int strreturn=0;
			try
			{
				string sql1="select count(*) from tbEmpSchLog where vcSchID='"+strSchID+"' and vcCardID='"+strCardID+"' and vcClass='"+strClass+"'";
				drr=SqlHelper.ExecuteReader(con,CommandType.Text,sql1);
				drr.Read();
				strreturn=int.Parse(drr[0].ToString());
				drr.Close();
		...
		public DataSet GetEmpSchList(Hashtable htPara)
		{
			...
			{
				string sql1="";
				if(htPara["strClass"].ToString()=="È«ÈÕ°à")
				{
				...
				{
					sql1="select vcEmpName from tbEmployee where vcFlag='0' and vcDeptID='"+htPara["strDeptID"].ToString()+"' and vcOfficer='"+htPara["strOfficerID"].ToString() +"'";
					sql1+=" and vcEmpName not in(select vcEmpName from tbEmpSchLog where vcSchID='"+htPara["strSchID"].ToString()+"' and vcDeptName='"+htPara["strDeptName"].ToString()+"' and vcClass='È«ÈÕ°à' and vcEmpOF='"+htPara["strOfficer"].ToString()+"')";
				}
		...
		public DataTable GetEmpSchLog(string strShcID)
		{
			...
			try
			{
				...
				string sql1="select distinct vcDeptName,'"+strSchDate+"' as SchDate,'1' as IsSch from tbEmpSchLog where vcSchID='"+ strShcID+"'";
				sql1+=" union select distinct vcCommName as vcDeptName,'"+strSchDate+"' as SchDate,'0' as IsSch from tbCommCode where vcCommSign='MD' and vcCommCode<>'CEN00'";
		...
		public DataTable GetEveryEmpSchLog(string strShcID,string strDeptID,string strDeptName)
		{
			...
			try
			{
				string sql1="select vcCardID,vcEmpName,b.vcCommName as vcEmpOF,'' as vcClass,'0' as seq from tbEmployee a,tbCommCode b where vcDeptID='"+strDeptID+"' and vcFlag='0' and vcCardID+vcEmpName not in(select vcCardID+vcEmpName from tbEmpSchLog where vcSchID='"+strShcID+"' and vcDeptName='"+strDeptName+"') and b.vcCommSign='OF' and a.vcOfficer=b.vcCommCode";
				sql1+=" union select a.vcCardID,a.vcEmpName,a.vcEmpOF,a.vcClass,b.vcCommCode as seq from tbEmpSchLog a,tbCommCode b where a.vcSchID='"+strShcID+"' and a.vcDeptName='"+strDeptName+"' and b.vcCommSign='EC' and a.vcClass=b.vcCommName order by vcCardID,vcEmpName,vcEmpOF,seq";
				dtShc=SqlHelper.ExecuteDataTable(con,CommandType.Text,sql1);
			}
			catch (Exception e) 
			{
		...
		public DataTable GetDeptSchDetail(string strDeptName,string strShcID)
		{
			DataTable dttmp=new DataTable();
			try
			{
				string sql1="select vcEmpOF,vcClass,vcEmpName,dtCheckIn,dtCheckOut,seq from (select a.vcEmpOF,a.vcClass,a.vcEmpName,convert(char(19),a.dtCheckIn,120) as dtCheckIn,";
				sql1+="convert(char(19),a.dtCheckOut,120) as dtCheckOut,b.vcCommCode as seq from tbEmpSchLog a,tbCommCode b,tbCommCode c where a.vcSchID='"+strShcID+"'";
				sql1+=" and a.vcDeptName='"+strDeptName+"' and b.vcCommSign='EC' and c.vcCommSign='OF' and a.vcEmpOF=c.vcCommName and a.vcClass=b.vcCommName union all";
				sql1+=" select c.vcCommName as vcEmpOF,b.vcCommName as vcClass,'','','',b.vcCommCode as seq from tbCommCode b,tbCommCode c where b.vcCommSign='EC'";
				sql1+=" and c.vcCommSign='OF' and c.vcCommName+b.vcCommName not in(select distinct vcEmpOF+vcClass from tbEmpSchLog where vcSchID='"+strShcID+"'";
				sql1+=" and vcDeptName='"+strDeptName+"')) q order by vcEmpOF,seq,dtCheckIn";
		...
		public DataSet GetSignSumQuery(Hashtable htpara)
		{
			...
			try
			{
				...
				{
					sql1="select 1 as sno,'³Ùµ½' as type,count(distinct vcEmpName) as TolEmpCount,count(*) as TolCount,'' as EmpList from tbSignList where vcSignDate between '"+htpara["strBegin"].ToString()+"' and '"+htpara["strEnd"].ToString()+"' and substring(vcSignState,1,1)='1'";
					sql1+=" union select 2 as sno,'ÔçÍË' as type,count(distinct vcEmpName) as TolEmpCount,count(*) as TolCount,'' as EmpList from tbSignList where vcSignDate between '"+htpara["strBegin"].ToString()+"' and '"+htpara["strEnd"].ToString()+"' and substring(vcSignState,2,1)='1'";
					sql1+=" union select 3 as sno,'²¡¼Ù' as type,count(distinct vcEmpName) as TolEmpCount,count(*) as TolCount,'' as EmpList from tbSignList where vcSignDate between '"+htpara["strBegin"].ToString()+"' and '"+htpara["strEnd"].ToString()+"' and substring(vcSignState,3,1)='1'";
					sql1+=" union select 4 as sno,'ÊÂ¼Ù' as type,count(distinct vcEmpName) as TolEmpCount,count(*) as TolCount,'' as EmpList from tbSignList where vcSignDate between '"+htpara["strBegin"].ToString()+"' and '"+htpara["strEnd"].ToString()+"' and substring(vcSignState,4,1)='1'";
					sql1+=" union select 5 as sno,'ÐÝÏ¢' as type,count(distinct vcEmpName) as TolEmpCount,count(*) as TolCount,'' as EmpList from tbSignList where vcSignDate between '"+htpara["strBegin"].ToString()+"' and '"+htpara["strEnd"].ToString()+"' and substring(vcSignState,9,1)='1'";
					sql1+=" union select 6 as sno,'È±ÇÚ' as type,count(distinct vcEmpName) as TolEmpCount,count(*) as TolCount,'' as EmpList from tbSignList where vcSignDate between '"+htpara["strBegin"].ToString()+"' and '"+htpara["strEnd"].ToString()+"' and substring(vcSignState,10,1)='1'";
					sql1+=" order by sno";					}
				else
				{
					sql1="select 1 as sno,'³Ùµ½' as type,count(distinct vcEmpName) as TolEmpCount,count(*) as TolCount,'' as EmpList from tbSignList where vcSignDate between '"+htpara["strBegin"].ToString()+"' and '"+htpara["strEnd"].ToString()+"' and vcDept = '"+htpara["strDeptName"].ToString()+"' and substring(vcSignState,1,1)='1'";
					sql1+=" union select 2 as sno,'ÔçÍË' as type,count(distinct vcEmpName) as TolEmpCount,count(*) as TolCount,'' as EmpList from tbSignList where vcSignDate between '"+htpara["strBegin"].ToString()+"' and '"+htpara["strEnd"].ToString()+"' and vcDept = '"+htpara["strDeptName"].ToString()+"' and substring(vcSignState,2,1)='1'";
					sql1+=" union select 3 as sno,'²¡¼Ù' as type,count(distinct vcEmpName) as TolEmpCount,count(*) as TolCount,'' as EmpList from tbSignList where vcSignDate between '"+htpara["strBegin"].ToString()+"' and '"+htpara["strEnd"].ToString()+"' and vcDept = '"+htpara["strDeptName"].ToString()+"' and substring(vcSignState,3,1)='1'";
					sql1+=" union select 4 as sno,'ÊÂ¼Ù' as type,count(distinct vcEmpName) as TolEmpCount,count(*) as TolCount,'' as EmpList from tbSignList where vcSignDate between '"+htpara["strBegin"].ToString()+"' and '"+htpara["strEnd"].ToString()+"' and vcDept = '"+htpara["strDeptName"].ToString()+"' and substring(vcSignState,4,1)='1'";
					sql1+=" union select 5 as sno,'ÐÝÏ¢' as type,count(distinct vcEmpName) as TolEmpCount,count(*) as TolCount,'' as EmpList from tbSignList where vcSignDate between '"+htpara["strBegin"].ToString()+"' and '"+htpara["strEnd"].ToString()+"' and vcDept = '"+htpara["strDeptName"].ToString()+"' and substring(vcSignState,9,1)='1'";
					sql1+=" union select 6 as sno,'È±ÇÚ' as type,count(distinct vcEmpName) as TolEmpCount,count(*) as TolCount,'' as EmpList from tbSignList where vcSignDate between '"+htpara["strBegin"].ToString()+"' and '"+htpara["strEnd"].ToString()+"' and vcDept = '"+htpara["strDeptName"].ToString()+"' and substring(vcSignState,10,1)='1'";
					sql1+=" order by sno";
				}
				dttmp=SqlHelper.ExecuteDataTable(con,CommandType.Text,sql1);
				dttmp.TableName="t1";
				...
				if(htpara["strDeptName"].ToString()=="")
				{
					sql2="select 1 as sno,vcEmpName,count(vcEmpName) as eCount from tbSignList where vcSignDate between '"+htpara["strBegin"].ToString()+"' and '"+htpara["strEnd"].ToString()+"' and substring(vcSignState,1,1)='1' group by vcEmpName";
					sql2+=" union select 2 as sno,vcEmpName,count(vcEmpName) as eCount from tbSignList where vcSignDate between '"+htpara["strBegin"].ToString()+"' and '"+htpara["strEnd"].ToString()+"' and substring(vcSignState,2,1)='1' group by vcEmpName";
					sql2+=" union select 3 as sno,vcEmpName,count(vcEmpName) as eCount from tbSignList where vcSignDate between '"+htpara["strBegin"].ToString()+"' and '"+htpara["strEnd"].ToString()+"' and substring(vcSignState,3,1)='1' group by vcEmpName";
					sql2+=" union select 4 as sno,vcEmpName,count(vcEmpName) as eCount from tbSignList where vcSignDate between '"+htpara["strBegin"].ToString()+"' and '"+htpara["strEnd"].ToString()+"' and substring(vcSignState,4,1)='1' group by vcEmpName";
					sql2+=" union select 5 as sno,vcEmpName,count(vcEmpName) as eCount from tbSignList where vcSignDate between '"+htpara["strBegin"].ToString()+"' and '"+htpara["strEnd"].ToString()+"' and substring(vcSignState,9,1)='1' group by vcEmpName";
				...
				{
					sql2="select 1 as sno,vcEmpName,count(vcEmpName) as eCount from tbSignList where vcSignDate between '"+htpara["strBegin"].ToString()+"' and '"+htpara["strEnd"].ToString()+"' and vcDept = '"+htpara["strDeptName"].ToString()+"' and substring(vcSignState,1,1)='1' group by vcEmpName";
					sql2+=" union select 2 as sno,vcEmpName,count(vcEmpName) as eCount from tbSignList where vcSignDate between '"+htpara["strBegin"].ToString()+"' and '"+htpara["strEnd"].ToString()+"' and vcDept = '"+htpara["strDeptName"].ToString()+"' and substring(vcSignState,2,1)='1' group by vcEmpName";
					sql2+=" union select 3 as sno,vcEmpName,count(vcEmpName) as eCount from tbSignList where vcSignDate between '"+htpara["strBegin"].ToString()+"' and '"+htpara["strEnd"].ToString()+"' and vcDept = '"+htpara["strDeptName"].ToString()+"' and substring(vcSignState,3,1)='1' group by vcEmpName";
					sql2+=" union select 4 as sno,vcEmpName,count(vcEmpName) as eCount from tbSignList where vcSignDate between '"+htpara["strBegin"].ToString()+"' and '"+htpara["strEnd"].ToString()+"' and vcDept = '"+htpara["strDeptName"].ToString()+"' and substring(vcSignState,4,1)='1' group by vcEmpName";
					sql2+=" union select 5 as sno,vcEmpName,count(vcEmpName) as eCount from tbSignList where vcSignDate between '"+htpara["strBegin"].ToString()+"' and '"+htpara["strEnd"].ToString()+"' and vcDept = '"+htpara["strDeptName"].ToString()+"' and substring(vcSignState,9,1)='1' group by vcEmpName";
		...
		public int SchedEmpDailyEvery(CMSMStruct.EmpSchLogStruct empsl)
		{
			...
			using(SqlTransaction tran=con.BeginTransaction())
			{
				try
				{
					...
					{
						string sql2="";
						if(empsl.strClass=="ÐÝÏ¢")
							sql2="insert into tbEmpSchLog values('"+ empsl.strSchID+"','"+empsl.strDeptName+"','"+empsl.strManager+"','"+empsl.strCardID+"','"+empsl.strEmpName+"','"+empsl.strEmpOF+"','"+empsl.strClass+"',null,null)";
						else
							sql2="insert into tbEmpSchLog values('"+ empsl.strSchID+"','"+empsl.strDeptName+"','"+empsl.strManager+"','"+empsl.strCardID+"','"+empsl.strEmpName+"','"+empsl.strEmpOF+"','"+empsl.strClass+"','"+empsl.strCheckIn+"','"+empsl.strCheckOut+"')";
		...
		public int IsErrorTimespanWithTwoClass(CMSMStruct.EmpSchLogStruct empschlog)
		{
			int schcount=-1;
			try
			{
				...
				TimeSpan tsinout;
				if(empschlog.strClass=="Ôç°à")
				{
					sql1="select top 1 * from tbEmpSchLog where vcSchID='"+empschlog.strSchID+"' and vcDeptName='"+empschlog.strDeptName+"' and vcCardID='"+empschlog.strCardID+"' and vcClass='ÖÐ°à'";
					dttmp=SqlHelper.ExecuteDataTable(con,CommandType.Text,sql1);
					...
					{
						DateTime dtNextClassSignIn=(DateTime)dttmp.Rows[0]["dtCheckIn"];
						DateTime dtClassSignOut=DateTime.Parse(empschlog.strCheckOut);
						tsinout=dtNextClassSignIn.Subtract(dtClassSignOut);
						if(tsinout.TotalMinutes<150)
						{
							schcount=1;
						}
						else
				...
				if(empschlog.strClass=="ÖÐ°à")
				{
					sql1="select top 1 * from tbEmpSchLog where vcSchID='"+empschlog.strSchID+"' and vcDeptName='"+empschlog.strDeptName+"' and vcCardID='"+empschlog.strCardID+"' and vcClass='Ôç°à'";
					dttmp=SqlHelper.ExecuteDataTable(con,CommandType.Text,sql1);
					if(dttmp.Rows.Count==0)
					{
						sql1="select top 1 * from tbEmpSchLog where vcSchID='"+empschlog.strSchID+"' and vcDeptName='"+empschlog.strDeptName+"' and vcCardID='"+empschlog.strCardID+"' and vcClass='Íí°à'";
						dttmp=SqlHelper.ExecuteDataTable(con,CommandType.Text,sql1);
						if(dttmp.Rows.Count==0)
						{
							schcount=0;
					...
					{
						DateTime dtLastClassSignOut=(DateTime)dttmp.Rows[0]["dtCheckOut"];
						DateTime dtClassSignIn=DateTime.Parse(empschlog.strCheckIn);
						tsinout=dtClassSignIn.Subtract(dtLastClassSignOut);
						if(tsinout.TotalMinutes<150)
						{
							schcount=1;
						}
