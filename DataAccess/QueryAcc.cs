...
//		SqlDataReader drr;
		...
		public DataTable GetTopQuery(Hashtable htPara,string strtype)
		{
			...
			try
			{
				...
				if(strtype=="0")
				{
					if(htPara["strDeptID"].ToString()!="")
					{
						sql1="select b.vcGoodsName,sum(iCount) as salecount from vwConsItem a,tbGoods b where a.vcGoodsID=b.vcGoodsID and a.dtConsDate between '" + htPara["strBegin"].ToString() +"' and '" + htPara["strEnd"].ToString() +" 23:59:59' and a.vcDeptID like '" + htPara["strDeptID"].ToString() + "' group by vcGoodsName order by sum(iCount) desc";
					}
					else
					{
						sql1="select b.vcGoodsName,sum(iCount) as salecount from vwConsItem a,tbGoods b where a.vcGoodsID=b.vcGoodsID and a.dtConsDate between '" + htPara["strBegin"].ToString() +"' and '" + htPara["strEnd"].ToString() +" 23:59:59' group by vcGoodsName order by sum(iCount) desc";
					}
				...
				else
				{
					if(htPara["strDeptID"].ToString()!="")
					{
						sql1="select a.vcCardID,vcAssName,sum(nFee) as salefee from vwConsItem a,tbAssociator b where b.vcAssType<>'AT002' and a.vcCardID=b.vcCardID and a.dtConsDate between '" + htPara["strBegin"].ToString() +"' and '" + htPara["strEnd"].ToString() +" 23:59:59' and a.vcDeptID like '" + htPara["strDeptID"].ToString() + "' and a.vcCardID<>'V9999' group by a.vcCardID,vcAssName order by sum(nFee) desc";
					}
					else
					{
						sql1="select a.vcCardID,vcAssName,sum(nFee) as salefee from vwConsItem a,tbAssociator b where b.vcAssType<>'AT002' and a.vcCardID=b.vcCardID and a.dtConsDate between '" + htPara["strBegin"].ToString() +"' and '" + htPara["strEnd"].ToString() +" 23:59:59' and a.vcCardID<>'V9999' group by a.vcCardID,vcAssName order by sum(nFee) desc";
					}
		...
		{
			string strsql="select * from tbAssociator a,tbCommCode b where a.iAssID=" + strAssid + " and a.vcCardID='"+strCardID+"' and b.vcCommSign='AT' and a.vcAssType=b.vcCommCode";
		...
		public DataTable GetSpecConsQuery(Hashtable htPara)
		{
			...
			try
			{
				...
				string sql1="select a.vcConsType,c.vcGoodsName,sum(b.iCount) as tolCount,sum(b.iCount*c.nPrice) as tolfee,sum(b.nFee) as tolcash from tbBillOther a,tbConsItemOther b,tbGoods c where a.iSerial=b.iSerial and a.vcConsType  in('PT003','PT004','PT005','PT006','PT007')";
